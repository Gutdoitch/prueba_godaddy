...
function edd_free_download_maybe_unhook_auto_register() {
	if ( ! is_user_logged_in() && edd_get_option( 'edd_free_downloads_bypass_auto_register', false ) && class_exists( 'EDD_Auto_Register' ) ) {
		remove_action( 'edd_auto_register_insert_user', array( EDD_Auto_Register::get_instance(), 'email_notifications' ), 10 );
		remove_action( 'edd_insert_payment', array( EDD_Auto_Register::get_instance(), 'maybe_insert_user' ), 10 );
		remove_action( 'edd_payment_saved', array( EDD_Auto_Register::get_instance(), 'maybe_insert_user' ), 10 );
	}
}
...
function edd_free_download_process() {
	...
	// Based on the user found, set a user ID.
	$user_id = $user ? $user->ID : 0;

	// Now determine if we have a customer with this information.
	if ( ! empty( $user_id ) ) {
	...
	if ( edd_get_option( 'edd_free_downloads_user_registration', false ) && ! is_user_logged_in() && ! class_exists( 'EDD_Auto_Register' ) ) {
		...
		$user_id = edd_register_and_login_new_user( $account );
	}
	...
	$payment->customer_id = ! empty( $customer->id ) ? $customer->id : 0;
	$payment->email       = ! empty( $customer->id ) ? $customer->email : $email;
	$payment->total       = 0;
...
function edd_free_downloads_process_auto_download() {
	...
	if ( isset( $_GET['payment-id'] ) ) {
		...
		if ( isset( $price_ids ) && is_array( $price_ids ) && ! empty( $price_ids ) ) {
			foreach ( $price_ids as $cart_id => $price_id ) {
				if ( ! edd_is_free_download( $download_id, $price_id ) ) {
					wp_die( __( 'The requested product is not a free product! Please try again or contact support.', 'edd-free-downloads' ), __( 'Oops!', 'edd-free-downloads' ) );
				}
			...
			if ( false === $require_new_payment && true === $has_purchased ) {
				...
				if ( function_exists( 'edd_get_order_items' ) ) {
					...
					if ( $orders ) {
						...
						$payment_id = ! empty( $order_items[0] ) ? $order_items[0]->order_id : false;
					 ...
					 * parameters as edd_get_payments
					 */
					$args = array(
