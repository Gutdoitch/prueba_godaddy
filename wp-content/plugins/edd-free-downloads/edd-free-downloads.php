...
if ( ! class_exists( 'EDD_Free_Downloads' ) ) {
	...
	class EDD_Free_Downloads {
		...
		private function setup_constants() {
			// Plugin version
			define( 'EDD_FREE_DOWNLOADS_VER', '2.3.10.2' );
		 ...
		 * @access      private
		 * @since       1.0.0
		 * @since       2.3.10.1 Checks for the EDD ExtensionRegistry
		 * @return      void
		 */
		private function hooks() {
			if ( class_exists( '\\EDD\\Extensions\\ExtensionRegistry' ) ) {
				add_action( 'edd_extension_license_init', function( \EDD\Extensions\ExtensionRegistry $registry ) {
					$registry->addExtension( __FILE__, 'Free Downloads', 453004, EDD_FREE_DOWNLOADS_VER, null );
				} );
			} elseif ( class_exists( 'EDD_License' ) ) {
				$license = new EDD_License( __FILE__, 'Free Downloads', EDD_FREE_DOWNLOADS_VER, 'Easy Digital Downloads', null, null, 453004 );
			}
		}
