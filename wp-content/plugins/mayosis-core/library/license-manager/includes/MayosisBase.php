<?php
if(!class_exists("MayosisBase")) {
	class MayosisBase {
    	public $key = "9D1F5B3CE8759CCC";
    	private $product_id = "1";
    	private $product_base = "mayosis";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_mayosis_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=mayosis_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
