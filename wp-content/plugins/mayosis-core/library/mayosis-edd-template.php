...
$wholesitetem= get_theme_mod( 'whole_site_product_template','');
 ...
 if ($templateautomationtype == 'allcat'){
    ...
    function mayosis_download_template( $template ) {
        if ( is_singular( 'download' ) && empty( $_GET['edd-free-download'] )  ) {
    ...
    function mayosis_download_template( $template ) {
        ...
        return $template;
         ...
         } elseif( $wholesitetem == 'audio' ) {
          add_filter( 'template_include', 'mayosis_download_template', 99 );

    function mayosis_download_template( $template ) {
        if ( is_singular( 'download' ) && empty( $_GET['edd-free-download'] )  ) {
            // change the default-download-template.php to your template name
            $default_template = locate_template( array( 'audio_template.php' ) );
            if ( '' != $default_template ) {
                return $default_template ;
            }
        }
    ...
    function mayosis_download_template( $template ) {
        if ( is_singular( 'download' ) && empty( $_GET['edd-free-download'] )  ) {
            // change the default-download-template.php to your template name
            $default_template = locate_template( array( 'custom-product-template.php' ) );
            if ( '' != $default_template ) {
                return $default_template ;
            }
        }
