...
if($imageeffect=='enable'){
        ...
        ?>
            ...
            <div class="ajax-posts__post" id="edd_download_<?php the_ID(); ?>">
            <?php
            ...
            if ($fpstyle=="one"){
               ...
               get_template_part('includes/products/style/product-style-one');
                ...
                get_template_part('includes/products/style/product-style-two-filter');

            } elseif ($fpstyle=="three"){
    ...
    <?php if (!$this->is_last_page($query)) : ?>
        ...
        <div class="ajax-posts__load-more w-full start-0 mt-5">
            <button class="js-load-more" data-page="<?= $this->get_page_number($query) + 1; ?>">
                <?php _e('Load more', 'ttc-ajax-filter') ?> 
            </button>
        </div>
    <?php endif; ?>
<?php else: ?>
    <div class="ajax-posts-message ajax-posts-message--empty">
        ...
        <h4><?php printf( __('Oh, we couldn\'t find any %s', 'ttc-ajax-filter'), $plural_post_name); ?></h4>
        <p><?php printf( __('Try different category or <a %s> reset them all.</a>', 'ttc-ajax-filter'), 'href="#" class="js-reset-filters"'); ?></p>
    </div>
<?php endif; ?>
