...
use EDDAmazon\Vendor\Aws\Arn\S3\AccessPointArn;
use EDDAmazon\Vendor\Aws\Exception\MultipartUploadException;
use EDDAmazon\Vendor\Aws\Result;
use EDDAmazon\Vendor\Aws\S3\Exception\S3Exception;
use EDDAmazon\Vendor\GuzzleHttp\Promise\Coroutine;
    ...
    private function validateLocation(array $location)
    {
        if (empty($location['Bucket']) || empty($location['Key'])) {
            throw new \InvalidArgumentException('Locations provided to an'
                . ' EDDAmazon\Vendor\Aws\S3\ObjectCopier must have a non-empty Bucket and Key');
        }
    }
