...
use EDDAmazon\Vendor\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
...
class Transfer implements PromisorInterface
{
     ...
     *   callback accepts a single argument: EDDAmazon\Vendor\Aws\CommandInterface $command.
    ...
    private function createDownloadPromise()
    {
        ...
        return (new EDDAmazon\Vendor\Aws\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \EDDAmazon\Vendor\Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return EDDAmazon\Vendor\Aws\filter(
                EDDAmazon\Vendor\Aws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = EDDAmazon\Vendor\Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return EDDAmazon\Vendor\Aws\filter($files, function ($key) {
