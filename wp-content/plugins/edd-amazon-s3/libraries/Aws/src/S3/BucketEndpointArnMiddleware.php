...
use EDDAmazon\Vendor\Aws\Arn\ArnParser;
use EDDAmazon\Vendor\Aws\Arn\ObjectLambdaAccessPointArn;
...
use EDDAmazon\Vendor\Aws\Arn\S3\OutpostsAccessPointArn;
use EDDAmazon\Vendor\Aws\Arn\S3\MultiRegionAccessPointArn;
use EDDAmazon\Vendor\Aws\Arn\S3\OutpostsArnInterface;
...
use EDDAmazon\Vendor\Aws\Exception\UnresolvedEndpointException;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(
        ...
        $region = \EDDAmazon\Vendor\Aws\strip_fips_pseudo_regions($region);
     ...
     *
     * @param $arn
     * @return \EDDAmazon\Vendor\Aws\Endpoint\Partition
     */
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!($clientPart->isRegionMatch($this->region, 's3'))) {
                $clientPart = $this->partitionProvider->getPartition(
                    \EDDAmazon\Vendor\Aws\strip_fips_pseudo_regions($this->region),
                    's3'
                );
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
