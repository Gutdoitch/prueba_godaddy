...
use EDDAmazon\Vendor\GuzzleHttp\Promise\EachPromise;
...
class CommandPool implements PromisorInterface
{
            ...
            foreach ($commands as $key => $command) {
                if (!($command instanceof CommandInterface)) {
                    throw new \InvalidArgumentException('Each value yielded by '
                        . 'the iterator must be an EDDAmazon\Vendor\Aws\CommandInterface.');
                }
                if ($before) {
                    $before($command, $key);
                }
     ...
     *
     * @return array
     * @see \EDDAmazon\Vendor\Aws\CommandPool::__construct for available configuration options.
     */
