...
use EDDAmazon\Vendor\Aws\Api\Validator;
use EDDAmazon\Vendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use EDDAmazon\Vendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use EDDAmazon\Vendor\Aws\ClientSideMonitoring\Configuration;
use EDDAmazon\Vendor\Aws\Configuration\ConfigurationResolver;
use EDDAmazon\Vendor\Aws\Credentials\CredentialProvider;
use EDDAmazon\Vendor\Aws\Credentials\Credentials;
use EDDAmazon\Vendor\Aws\Credentials\CredentialsInterface;
use EDDAmazon\Vendor\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use EDDAmazon\Vendor\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use EDDAmazon\Vendor\Aws\Endpoint\EndpointProvider;
use EDDAmazon\Vendor\Aws\Endpoint\PartitionEndpointProvider;
use EDDAmazon\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use EDDAmazon\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use EDDAmazon\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use EDDAmazon\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use EDDAmazon\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use EDDAmazon\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use EDDAmazon\Vendor\Aws\EndpointDiscovery\ConfigurationInterface;
use EDDAmazon\Vendor\Aws\EndpointDiscovery\ConfigurationProvider;
use EDDAmazon\Vendor\Aws\EndpointV2\EndpointDefinitionProvider;
use EDDAmazon\Vendor\Aws\Exception\AwsException;
use EDDAmazon\Vendor\Aws\Exception\InvalidRegionException;
use EDDAmazon\Vendor\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use EDDAmazon\Vendor\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use EDDAmazon\Vendor\Aws\Signature\SignatureProvider;
use EDDAmazon\Vendor\Aws\Token\Token;
use EDDAmazon\Vendor\Aws\Token\TokenInterface;
use EDDAmazon\Vendor\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    private static $defaultArgs = [
        'service' => [
            'type'     => 'value',
            'valid'    => ['string'],
            'doc'      => 'Name of the service to utilize. This value will be supplied by default when using one of the SDK clients (e.g., EDDAmazon\Vendor\Aws\\S3\\S3Client).',
            'required' => true,
            'internal' => true
        ],
        'exception_class' => [
        ...
        'signature_provider' => [
            'type'    => 'value',
            'valid'   => ['callable'],
            'doc'     => 'A callable that accepts a signature version name (e.g., "v4"), a service name, and region, and  returns a SignatureInterface object or null. This provider is used to create signers utilized by the client. See EDDAmazon\Vendor\Aws\\Signature\\SignatureProvider for a list of built-in providers',
        ...
        'configuration_mode' => [
            ...
            'doc'     => "Sets the default configuration mode. Otherwise provide an instance of EDDAmazon\Vendor\Aws\DefaultsMode\ConfigurationInterface, an instance of  EDDAmazon\Vendor\Aws\CacheInterface, or a string containing a valid mode",
        ...
        'endpoint_provider' => [
            ...
            'doc'      => 'An optional PHP callable that accepts a hash of options including a "service" and "region" key and returns NULL or a hash of endpoint data, of which the "endpoint" key is required. See EDDAmazon\Vendor\Aws\\Endpoint\\EndpointProvider for a list of built-in providers.',
        ...
        'credentials' => [
            ...
            'doc'     => 'Specifies the credentials used to sign requests. Provide an EDDAmazon\Vendor\Aws\Credentials\CredentialsInterface object, an associative array of "key", "secret", and an optional "token" key, `false` to use null credentials, or a callable credentials provider used to create credentials or return null. See EDDAmazon\Vendor\Aws\\Credentials\\CredentialProvider for a list of built-in credentials providers. If no credentials are provided, the SDK will attempt to load them from the environment.',
        ...
        'token' => [
            ...
            'doc'     => 'Specifies the token used to authorize requests. Provide an EDDAmazon\Vendor\Aws\Token\TokenInterface object, an associative array of "token", and an optional "expiration" key, `false` to use a null token, or a callable token provider used to fetch a token or return null. See EDDAmazon\Vendor\Aws\\Token\\TokenProvider for a list of built-in credentials providers. If no token is provided, the SDK will attempt to load one from the environment.',
        ...
        'endpoint_discovery' => [
            ...
            'doc'      => 'Specifies settings for endpoint discovery. Provide an instance of EDDAmazon\Vendor\Aws\EndpointDiscovery\ConfigurationInterface, an instance EDDAmazon\Vendor\Aws\CacheInterface, a callable that provides a promise for a Configuration object, or an associative array with the following keys: enabled: (bool) Set to true to enable endpoint discovery, false to explicitly disable it. Defaults to false; cache_limit: (int) The maximum number of keys in the endpoints cache. Defaults to 1000.',
        ...
        'stats' => [
            ...
            'valid' => ['bool', 'array'],
            'default' => false,
            'doc'   => 'Set to true to gather transfer statistics on requests sent. Alternatively, you can provide an associative array with the following keys: retries: (bool) Set to false to disable reporting on retries attempted; http: (bool) Set to true to enable collecting statistics from lower level HTTP adapters (e.g., values returned in EDDAmazon\Vendor\GuzzleHttp\TransferStats). HTTP handlers must support an http_stats_receiver option for this to have an effect; timer: (bool) Set to true to enable a command timer that reports the total wall clock time spent on an operation in seconds.',
            'fn'    => [__CLASS__, '_apply_stats'],
        ],
        'retries' => [
            'type'    => 'value',
            'valid'   => ['int', RetryConfigInterface::class, CacheInterface::class, 'callable', 'array'],
            'doc'     => "Configures the retry mode and maximum number of allowed retries for a client (pass 0 to disable retries). Provide an integer for 'legacy' mode with the specified number of retries. Otherwise provide an instance of EDDAmazon\Vendor\Aws\Retry\ConfigurationInterface, an instance of  EDDAmazon\Vendor\Aws\CacheInterface, a callable function, or an array with the following keys: mode: (string) Set to 'legacy', 'standard' (uses retry quota management), or 'adapative' (an experimental mode that adds client-side rate limiting to standard mode); max_attempts: (int) The maximum number of attempts for a given request. ",
        ...
        'csm' => [
            'type'     => 'value',
            'valid'    => [\EDDAmazon\Vendor\Aws\ClientSideMonitoring\ConfigurationInterface::class, 'callable', 'array', 'bool'],
            ...
            'default'  => [\EDDAmazon\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::class, 'defaultProvider']
        ],
        'http' => [
            'type'    => 'value',
            'valid'   => ['array'],
        ...
        'handler' => [
            'type'     => 'value',
            'valid'    => ['callable'],
            'doc'      => 'A handler that accepts a command object, request object and returns a promise that is fulfilled with an EDDAmazon\Vendor\Aws\ResultInterface object or rejected with an EDDAmazon\Vendor\Aws\Exception\AwsException. A handler does not accept a next handler as it is terminal and expected to fulfill a command. If no handler is provided, a default Guzzle handler will be utilized.',
    ...
    public static function _apply_csm($value, array &$args, HandlerList $list)
    {
        if ($value === false) {
            $value = new Configuration(
                false,
                \EDDAmazon\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST,
                \EDDAmazon\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT,
                \EDDAmazon\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID
            );
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \EDDAmazon\Vendor\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \EDDAmazon\Vendor\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _apply_user_agent($inputUserAgent, array &$args, HandlerList $list)
    {
        ...
        if (isset($args['endpoint_discovery'])) {
            if (($args['endpoint_discovery'] instanceof \EDDAmazon\Vendor\Aws\EndpointDiscovery\Configuration
        ...
        if (isset($args['retries'])) {
            if ($args['retries'] instanceof \EDDAmazon\Vendor\Aws\Retry\Configuration) {
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName)
            ...
            return new \EDDAmazon\Vendor\Aws\EndpointV2\EndpointProviderV2(
                $ruleset,
    ...
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \EDDAmazon\Vendor\Aws\manifest($args['service'])['serviceIdentifier'];
...
EOT;
    ...
    private static function isValidService($service) {
        ...
        $services = \EDDAmazon\Vendor\Aws\manifest();
