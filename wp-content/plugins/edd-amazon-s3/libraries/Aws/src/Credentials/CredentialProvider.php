<?php
namespace EDDAmazon\Vendor\Aws\Credentials;

use EDDAmazon\Vendor\Aws;
use EDDAmazon\Vendor\Aws\Api\DateTimeResult;
use EDDAmazon\Vendor\Aws\CacheInterface;
use EDDAmazon\Vendor\Aws\Exception\CredentialsException;
use EDDAmazon\Vendor\Aws\Sts\StsClient;
use EDDAmazon\Vendor\GuzzleHttp\Promise;
/**
 * Credential providers are functions that accept no arguments and return a
 * promise that is fulfilled with an {@see \EDDAmazon\Vendor\Aws\Credentials\CredentialsInterface}
 * or rejected with an {@see \EDDAmazon\Vendor\Aws\Exception\CredentialsException}.
 *
 * <code>
 ...
 * {@see EDDAmazon\Vendor\Aws\Credentials\CredentialProvider::chain}. This function accepts
...
class CredentialProvider
{
     ...
     * instance of EDDAmazon\Vendor\Aws\CacheInterface. Forwards calls when no credentials found
     ...
     * @see EDDAmazon\Vendor\Aws\Credentials\InstanceProfileProvider for $config details.
     */
     ...
     * @see EDDAmazon\Vendor\Aws\Credentials\EcsCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see EDDAmazon\Vendor\Aws\Credentials\AssumeRoleCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see EDDAmazon\Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider for
     * $config details.
     */
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \EDDAmazon\Vendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: $filename");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \EDDAmazon\Vendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            $configProfileData = \EDDAmazon\Vendor\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles() {
        ...
        if (file_exists($credFile)) {
            $profiles = \EDDAmazon\Vendor\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }

        if (file_exists($configFile)) {
            $configProfileData = \EDDAmazon\Vendor\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    private static function getSsoCredentials($profiles, $ssoProfileName, $filename, $config)
    {
        if (empty($config['ssoOidcClient'])) {
            ...
            $ssoOidcClient = new EDDAmazon\Vendor\Aws\SSOOIDC\SSOOIDCClient([
        ...
        $tokenPromise = new EDDAmazon\Vendor\Aws\Token\SsoTokenProvider(
            $ssoProfileName,
            $filename,
            $ssoOidcClient
        );
    ...
    private static function getCredentialsFromSsoService($ssoProfile, $clientRegion, $accessToken, $config)
    {
        if (empty($config['ssoClient'])) {
            $ssoClient = new EDDAmazon\Vendor\Aws\SSO\SSOClient([
                'region' => $clientRegion,
                'version' => '2019-06-10',
                'credentials' => false
            ]);
