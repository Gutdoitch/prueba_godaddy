...
<table id="edd_purchase_receipt" class="edd-table">
	<thead>
		...
		<tr>
			<th><strong><?php echo esc_html_x( 'Order', 'heading', 'mayosis' ); ?>:</strong></th>
	...
	<tbody>
		<tr>
			<td class="edd_receipt_payment_status"><strong><?php esc_html_e( 'Order Status', 'mayosis' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $edd_receipt_args['payment_key'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php esc_html_e( 'Payment Key', 'mayosis' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $edd_receipt_args['payment_method'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php esc_html_e( 'Payment Method', 'mayosis' ); ?>:</strong></td>
		...
		<tr>
			<td><strong><?php esc_html_e( 'Date', 'mayosis' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $edd_receipt_args['price'], FILTER_VALIDATE_BOOLEAN ) && $order->subtotal > 0 ) : ?>
			<tr>
				<td><strong><?php esc_html_e( 'Subtotal', 'mayosis' ); ?>:</strong></td>
				<td>
		...
		if ( filter_var( $edd_receipt_args['discount'], FILTER_VALIDATE_BOOLEAN ) ) :
			...
			if ( $order_discounts ) :
				$label = _n( 'Discount', 'Discounts', count( $order_discounts ), 'mayosis' );
				?>
				<tr>
		...
		if ( ! empty( $fees ) ) :
			?>
			<tr>
				<td colspan="2"><strong><?php echo esc_html( _n( 'Fee', 'Fees', count( $fees ), 'mayosis' ) ); ?>:</strong></td>
			</tr>
			<?php
			foreach ( $fees as $fee ) :
				$label = __( 'Fee', 'mayosis' );
		...
		<?php if ( $order->tax > 0 ) : ?>
			<tr>
				<td><strong><?php esc_html_e( 'Tax', 'mayosis' ); ?>:</strong></td>
		...
		if ( $credits ) {
			?>
			<tr>
				<td colspan="2"><strong><?php echo esc_html( _n( 'Credit', 'Credits', count( $credits ), 'mayosis' ) ); ?>:</strong></td>
			</tr>
			<?php
			foreach ( $credits as $credit ) {
				$label = __( 'Credit', 'mayosis' );
		...
		<?php if ( filter_var( $edd_receipt_args['price'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php esc_html_e( 'Total', 'mayosis' ); ?>:</strong></td>
...
<h3><?php echo esc_html( apply_filters( 'edd_payment_receipt_products_title', __( 'Products', 'mayosis' ) ) ); ?></h3>
				...
				<td>
					...
					<ul class="edd_purchase_receipt_files">
						...
						elseif ( edd_is_bundled_product( $item->product_id ) ) :
								...
								<li class="edd_bundled_product">
									...
									<ul class="edd_bundled_product_files">
										...
										if ( $download_files && is_array( $download_files ) ) :
											...
											echo '<li>' . esc_html__( 'No downloadable files found for this bundled item.', 'mayosis' ) . '</li>';
										endif;
										?>
									</ul>
								</li>
							...
							echo '<li>' . esc_html( apply_filters( 'edd_receipt_no_files_found_text', __( 'No downloadable files found.', 'mayosis' ), $item->product_id ) ) . '</li>';
						endif;
						?>
					</ul>
					<?php endif; ?>
